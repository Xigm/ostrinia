...
class Predictor(Predictor):
    ...
    def validation_step(self, batch: Union[Tensor, Dict[str, Tensor]], batch_idx: int = 0) -> Union[Tensor, Dict[str, Tensor]]:
        """
        Perform a validation step on the input batch.

        Args:
            batch (Union[Tensor, Dict[str, Tensor]]): Input data for validation.
            batch_idx (int): Index of the batch.

        Returns:
            Union[Tensor, Dict[str, Tensor]]: Loss value or a dictionary containing loss and embeddings.
        """
        return super().validation_step(batch, batch_idx=batch_idx)
    
    def test_step(self, batch: Union[Tensor, Dict[str, Tensor]], batch_idx: int = 0) -> Union[Tensor, Dict[str, Tensor]]:
        """
        Perform a test step on the input batch.

        Args:
            batch (Union[Tensor, Dict[str, Tensor]]): Input data for testing.
            batch_idx (int): Index of the batch.

        Returns:
            Union[Tensor, Dict[str, Tensor]]: Loss value or a dictionary containing loss and embeddings.
        """
        return super().test_step(batch, batch_idx=batch_idx)
